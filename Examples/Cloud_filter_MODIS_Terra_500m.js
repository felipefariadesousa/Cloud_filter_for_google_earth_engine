...
//Function to apply scale bands in MODIS-Terra
function scale_bands (image) {
  var opticalBands = image.select('sur_refl_b.*').multiply(0.0001)
  return image.addBands(opticalBands, null, true).copyProperties(image, ['system:time_start'])}

//Remove pixels with negative values
function remove_pixels_negative (image){
  var image_without_pixels_negative = image
  image_without_pixels_negative = image_without_pixels_negative.updateMask(image_without_pixels_negative.select('sur_refl_b03').gte(0)) //Blue band
  image_without_pixels_negative = image_without_pixels_negative.updateMask(image_without_pixels_negative.select('sur_refl_b04').gte(0)) //Green band
  image_without_pixels_negative = image_without_pixels_negative.updateMask(image_without_pixels_negative.select('sur_refl_b01').gte(0)) //Red band
  image_without_pixels_negative = image_without_pixels_negative.updateMask(image_without_pixels_negative.select('sur_refl_b05').gte(0)) //Near InfraRed band
  
  return image.addBands(image_without_pixels_negative, null, true).copyProperties(image, ['system:time_start'])
}

//Function Cloud Mask
var cloud_mask = function(image) {
 var cld = require('users/fitoprincipe/geetools:cloud_masks') //Repository from the code to cloud mask
 var mask = cld.modisSR(['cirrus','cloud','cloud2','shadow'])(image) //apply cloud mask
 return mask.copyProperties(image, ['system:time_start'])
}

//Function Land Mask
function mask_water(image){
  var ndwi = image.normalizedDifference(['sur_refl_b04', 'sur_refl_b01'])
  return image.updateMask(ndwi.gt(0)).copyProperties(image, ['system:time_start'])
}

//Function cloud filter
function filter_clouds_modis (image) {
  var cld = require('users/fitoprincipe/geetools:cloud_masks') //Repository from the code to cloud mask
  var mask = cld.modisSR(['cirrus','cloud','cloud2','shadow'])(image) //apply cloud mask
  
  var image_without_cloud_mask = image.select('sur_refl_b01') //image without cloud mask
  var image_with_cloud_mask = mask.select('sur_refl_b01') //image with cloud mask
  
  //You can use any band. In this example, I am using sur_refl_b01. 
  ...
  var count_image_without_cloud_mask = image_without_cloud_mask.reduceRegion(ee.Reducer.count(),cartagena_bay,500,null,null,false,1e13).get('sur_refl_b01')
  ...
  var count_image_with_cloud_mask = image_with_cloud_mask.reduceRegion(ee.Reducer.count(),cartagena_bay,500,null,null,false,1e13).get('sur_refl_b01')
...
//Function to Deglint
function deglint (image) {
  
  var time_start = image.copyProperties(image, ['system:time_start']) //For copy proprietes time_sytem
  
  //Apply linear regression between visible bands and NIR with shortwave infrared
	var linearFitBlue = image.select(['sur_refl_b05', 'sur_refl_b03']).reduceRegion({reducer: ee.Reducer.linearFit(), geometry: cartagena_bay, scale: 500});
	var linearFitGreen = image.select(['sur_refl_b05', 'sur_refl_b04']).reduceRegion({reducer: ee.Reducer.linearFit(), geometry: cartagena_bay, scale: 500});
	var linearFitRed = image.select(['sur_refl_b05', 'sur_refl_b01']).reduceRegion({reducer: ee.Reducer.linearFit(), geometry: cartagena_bay, scale: 500});
  var linearFitNir = image.select(['sur_refl_b05', 'sur_refl_b02']).reduceRegion({reducer: ee.Reducer.linearFit(), geometry: cartagena_bay, scale: 500});
  
  //Obtaning slope from linear regression
	var slopeImage = ee.Dictionary({'sur_refl_b03': linearFitBlue.get('scale'),
	'sur_refl_b04': linearFitGreen.get('scale'),
	'sur_refl_b01': linearFitRed.get('scale'),
	'sur_refl_b02': linearFitNir.get('scale')}).toImage().toFloat()

  //Obtaning minimum SWIR values
	var minSWIR = image.select('sur_refl_b05').reduceRegion(ee.Reducer.min(), cartagena_bay).toImage()

  //Apply method of Hedley et al., (2005)
	var image_deglint = image.select(['sur_refl_b03', 'sur_refl_b04', 'sur_refl_b01', 'sur_refl_b02']).subtract(slopeImage.multiply((image.select('sur_refl_b05').subtract(minSWIR))))
  
  //When subtracting the values by the minimum of the swir, 
  //there is a risk that some values will become negative
  //Therefore, you must ensure that negative values are removed again
  image_deglint = image_deglint.updateMask(image_deglint.select('sur_refl_b03').gte(0)) //Blue band
  image_deglint = image_deglint.updateMask(image_deglint.select('sur_refl_b04').gte(0)) //Green band
  image_deglint = image_deglint.updateMask(image_deglint.select('sur_refl_b01').gte(0)) //Red band
  image_deglint = image_deglint.updateMask(image_deglint.select('sur_refl_b02').gte(0)) //Near InfraRed
  
  return image.addBands(image_deglint, null, true).copyProperties(time_start, ['system:time_start'])
}

//I drew a square around the bay so as not to show the entire modis scene, just the bay region.
function clip (image){return image.clipToCollection(ee.FeatureCollection(geometry))}

//Image Collection MODIS-Terra 500m
var dataset = ee.ImageCollection('MODIS/061/MOD09GA')
                .filterDate('2016-01-01', '2017-01-01') //Range data 
                .map(clip) // Clip to cartagena bay
                .map(filter_clouds_modis) //Apply filter cloud
                .filterMetadata('CLOUD_COVER_BAY', 'less_than', 20) //Filtering cloud cover less than 20%
                .map(scale_bands) //Apply scale band of MODIS-Terra
                .map(cloud_mask) //Apply cloud mask
                .map(mask_water) //Apply land mask
                .map(deglint) //Apply glint correction

//Centering in cartagena bay
Map.setCenter(-75.54307476890797, 10.324208272791243)

print('All scenes from image collection filtered by cloud filter')
print(dataset)
